(function (global, factory) {
    ...
    (global = global || self, global.safetyResultsOverTime = factory(global.d3, global.webCharts));
    ...
    function syncSettings(settings) {
        ...
        settings.marks.filter(function (mark) {
    ...
    function controlInputs() {
        ...
        }, { type: 'checkbox', inline: true, option: 'boxplots', label: 'Box plots' }, { type: 'checkbox', inline: true, option: 'violins', label: 'Violin plots' }, { type: 'checkbox', inline: true, option: 'outliers', label: 'Outliers' }];
    }
    ...
    function addVariables() {
        ...
        this.raw_data.forEach(function (d) {
            ...
            d.srot_measure = d.hasOwnProperty(_this.config.unit_col) ? d[_this.config.measure_col] + ' (' + d[_this.config.unit_col] + ')' : d[_this.config.measure_col];
    ...
    function checkGroupByVariables() {
        ...
        this.config.groups = this.config.groups.filter(function (group) {
            ...
            if (!groupByExists) console.warn('The [ ' + group.label + ' ] group-by option has been removed because the variable does not exist.');
    ...
    function defineMeasureSet() {
        ...
        this.srot_measures = this.initial_data[0].hasOwnProperty(this.config.measure_order_col) ? [].concat(toConsumableArray(new Set(this.initial_data.map(function (d) {
            return +d[_this.config.measure_order_col];
        })).values())).sort(function (a, b) {
            return a - b;
        }).map(function (value) {
    ...
    function setInitialMeasure() {
        ...
        if (this.config.start_value && this.srot_measures.indexOf(this.config.start_value) < 0 && this.measures.indexOf(this.config.start_value) < 0) {
    ...
    function classControlGroups() {
        ...
        this.controls.wrap.style('position', 'relative').selectAll('.control-group').style('vertical-align', 'middle').each(function (d, i) {
            var controlGroup = d3.select(this);
            controlGroup.classed(d.type.toLowerCase().replace(' ', '-') + ' ' + d.label.toLowerCase().replace(' ', '-'), true);
    ...
    function addYDomainResetButton() {
        var context = this,
            resetContainer = this.controls.wrap.insert('div', '.lower').classed('control-group y-axis', true).datum({
            type: 'button',
            option: 'y.domain',
            label: 'Limits'
        }),
            resetLabel = resetContainer.append('span').attr('class', 'wc-control-label').text('Limits'),
            resetButton = resetContainer.append('button').style('padding', '0px 5px').text('Reset').on('click', function () {
    ...
    function groupYAxisControls() {
        ...
        var grouping = this.controls.wrap.insert('div', '.y-axis').style({
        ...
        this.controls.wrap.selectAll('.y-axis').each(function (d) {
            ...
            if (d.option === 'y.type') d3.select(this).selectAll('input[type=radio]').style({
                top: '-.1em'
            });
        });
    }
    ...
    function defineMeasureData() {
        ...
        if (this.config.y.type === 'log') {
            ...
            if (nonPositiveResults > 0) this.controls.wrap.selectAll('.axis-type .radio').filter(function () {
                return d3.select(this).select('input').attr('value') === 'log';
            }).append('small').classed('non-positive-results', true).text(nonPositiveResults + ' nonpositive result' + (nonPositiveResults > 1 ? 's' : '') + ' removed.');
        }
        ...
        this.filters.forEach(function (filter) {
            _this.filtered_measure_data = _this.filtered_measure_data.filter(function (d) {
                return Array.isArray(filter.val) ? filter.val.indexOf(d[filter.col]) > -1 : filter.val === d[filter.col] || filter.val === 'All';
            });
        });
        ...
        this.nested_measure_data = d3.nest().key(function (d) {
            ...
            ['min', ['quantile', 0.05], ['quantile', 0.25], 'median', ['quantile', 0.75], ['quantile', 0.95], 'max', 'mean', 'deviation'].forEach(function (item) {
                var fx = Array.isArray(item) ? item[0] : item;
                var stat = Array.isArray(item) ? '' + fx.substring(0, 1) + item[1] * 100 : fx;
                results[stat] = Array.isArray(item) ? d3[fx](results.values, item[1]) : d3[fx](results.values);
            });
    ...
    function flagOutliers() {
        ...
        this.nested_measure_data.forEach(function (visit) {
            visit.values.forEach(function (group) {
                _this.quantileMap.set(visit.key + "|" + group.key, // key
                [group.values.q5, group.values.q95] // value
                );
            });
        });
        this.filtered_measure_data.forEach(function (d) {
            var quantiles = _this.quantileMap.get(d[_this.config.x.column] + "|" + d[_this.config.color_by]);
            d.srot_outlier = _this.config.outliers ? d[_this.config.y.column] < quantiles[0] || quantiles[1] < d[_this.config.y.column] : false;
        });
    }
    ...
    function setYdomain() {
        ...
        //Define y-domain.
        if (this.currentMeasure !== this.previousMeasure || this.currentYAxis !== this.previousYAxis) this.config.y.domain = d3.extent(this.measure_data.map(function (d) {
    ...
    function updateYaxisLimitControls() {
        ...
        this.controls.wrap.selectAll('.control-group').filter(function (f) {
            return f.option === 'y.domain[0]';
        }).select('input').attr('step', step).property('value', yDomain[0]);
        this.controls.wrap.selectAll('.control-group').filter(function (f) {
            return f.option === 'y.domain[1]';
        }).select('input').attr('step', step).property('value', yDomain[1]);
    }
    ...
    function removeUnscheduledVisits$1() {
        ...
        if (!this.config.unscheduled_visits) this.marks.forEach(function (mark) {
            if (mark.type === 'line') mark.data.forEach(function (d) {
                d.values = d.values.filter(function (di) {
                    return _this.config.x.domain.indexOf(di.key) > -1;
                });
            });else if (mark.type === 'circle') mark.data = mark.data.filter(function (d) {
    ...
    function drawLogAxis() {
        ...
        if (this.config.y.type === 'log') {
            var logYAxis = d3.svg.axis().scale(this.y).orient('left').ticks(8, ',' + this.config.y.format).tickSize(6, 0);
    ...
    function handleEmptyAxis() {
        ...
        if (this.svg.selectAll('.y .tick').size() < 2) {
            ...
            var probs = [{ probability: 0.1 }, { probability: 0.3 }, { probability: 0.5 }, { probability: 0.7 }, { probability: 0.9 }];
    ...
    function removeDuplicateTickLabels() {
        ...
        this.svg.selectAll('.y.axis .tick').each(function (d) {
            ...
            if (label.size()) {
                ...
                if (tickLabels.indexOf(tickLabel) < 0) tickLabels.push(tickLabel);else label.remove();
            }
        });
    }

    function fixFloatingPointIssues() {
        this.svg.selectAll('.y.axis .tick text').filter(function (d) {
    ...
    function defineScales(subgroup) {
          ...
          subgroup.boxplot.y = this.config.y.type === 'linear' ? d3.scale.linear().range([this.plot_height, 0]).domain(this.y.domain()) : d3.scale.log().range([this.plot_height, 0]).domain(this.y.domain());
    }

    function addContainer(subgroup) {
        subgroup.boxplot.container = subgroup.svg.append('g').attr('class', 'boxplot').datum({
    ...
    function drawBox(subgroup) {
        subgroup.boxplot.container.append('rect').attr({
            class: 'boxplot fill',
            x: subgroup.boxplot.left,
            width: subgroup.boxplot.right - subgroup.boxplot.left,
            y: subgroup.boxplot.y(subgroup.boxplot.probs[3]),
            height: subgroup.boxplot.y(subgroup.boxplot.probs[1]) - subgroup.boxplot.y(subgroup.boxplot.probs[3])
    ...
    function drawHorizontalLines(subgroup) {
        var iS = [0, 2, 4];
        var iSclass = ['', 'median', ''];
        var iSColor = [subgroup.boxplot.boxColor, subgroup.boxplot.boxInsideColor, subgroup.boxplot.boxColor];
        for (var i = 0; i < iS.length; i++) {
            subgroup.boxplot.container.append('line').attr({
    ...
    function drawOuterCircle(subgroup) {
        subgroup.boxplot.container.append('circle').attr({
            class: 'boxplot mean',
    ...
    function defineData(subgroup) {
        ...
        subgroup.violinPlot = {
            histogram: d3.layout.histogram().bins(10).frequency(0)
        };
        subgroup.violinPlot.data = subgroup.violinPlot.histogram(subgroup.results.values), subgroup.violinPlot.data.unshift({
    ...
    function defineScales$1(subgroup) {
        ...
        subgroup.violinPlot.x = this.config.y.type === 'linear' ? d3.scale.linear().domain(this.y.domain()).range([this.plot_height, 0]) : d3.scale.log().domain(this.y.domain()).range([this.plot_height, 0]);
        subgroup.violinPlot.y = d3.scale.linear().domain([0, Math.max(1 - 1 / subgroup.group.x.nGroups, d3.max(subgroup.violinPlot.data, function (d) {
            return d.y;
    ...
    function addContainer$1(subgroup) {
        ...
        subgroup.violinPlot.area = d3.svg.area().interpolate('basis').x(function (d) {
            return subgroup.violinPlot.x(d.x + d.dx / 2);
        }).y0(subgroup.violinPlot.width / 2).y1(function (d) {
            return subgroup.violinPlot.y(d.y);
        });
        subgroup.violinPlot.line = d3.svg.line().interpolate('basis').x(function (d) {
            return subgroup.violinPlot.x(d.x + d.dx / 2);
        }).y(function (d) {
            return subgroup.violinPlot.y(d.y);
        });
        subgroup.violinPlot.container = subgroup.svg.append('g').attr('class', 'violinplot').attr('clip-path', 'url(#' + this.id + ')');
    }

    function drawLeftSide(subgroup) {
        subgroup.violinPlot.gMinus = subgroup.violinPlot.container.append('g').attr('transform', 'rotate(90,0,0) scale(1,-1)');
        subgroup.violinPlot.gMinus.append('path').datum(subgroup.violinPlot.data).attr({
            class: 'area',
    ...
    function drawRightSide(subgroup) {
        subgroup.violinPlot.gPlus = subgroup.violinPlot.container.append('g').attr('transform', 'rotate(90,0,0) translate(0,-' + subgroup.violinPlot.width + ')');
        subgroup.violinPlot.gPlus.append('path').datum(subgroup.violinPlot.data).attr({
            class: 'area',
            d: subgroup.violinPlot.area,
            fill: this.colorScale(subgroup.key),
            'fill-opacity': 0.75
    ...
    function drawPlots() {
        ...
        this.nested_measure_data.filter(function (visit) {
            ...
            visit.values.forEach(function (group, i) {
                ...
                subgroup.svg = _this.svg.insert('g', '.point-supergroup').attr({
                    class: 'boxplot-wrap overlay-item',
                    transform: 'translate(' + (_this.x(groupObject.x.key) + subgroup.offset) + ',0)'
                ...
                //Offset outliers.
                _this.marks.filter(function (mark) {
                    return mark.type === 'circle';
                }).forEach(function (mark) {
                    mark.groups.filter(function (d) {
    ...
    function safetyResultsOverTime(element, settings) {
        ...
        var syncedControlInputs = configuration.syncControlInputs(configuration.controlInputs(), syncedSettings); //Sync merged settings with controls.
