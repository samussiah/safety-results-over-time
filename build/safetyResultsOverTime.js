(function (global, factory) {
    ...
    function syncSettings(settings) {
        ...
        if (!(settings.groups instanceof Array && settings.groups.length)) settings.groups = [{ value_col: 'srot_none', label: 'None' }];else settings.groups = settings.groups.map(function (group) {
            return {
    ...
    function controlInputs() {
        return [{
            ...
            require: true
        }, { type: 'number', label: 'Lower Limit', option: 'y.domain[0]', require: true }, { type: 'number', label: 'Upper Limit', option: 'y.domain[1]', require: true }, {
            type: 'checkbox',
            inline: true,
        ...
        }, { type: 'checkbox', inline: true, option: 'boxplots', label: 'Box plots' }, { type: 'checkbox', inline: true, option: 'violins', label: 'Violin plots' }, { type: 'checkbox', inline: true, option: 'outliers', label: 'Outliers' }, { type: 'radio', option: 'y.type', values: ['linear', 'log'], label: 'Axis type' }];
    }
    ...
    function defineVisitOrder() {
        ...
        if (this.config.time_settings.order_col && this.raw_data[0].hasOwnProperty(this.config.time_settings.order_col)) {
            ...
            visits = d3$1.set(this.raw_data.map(function (d) {
    ...
    function defineMeasureSet() {
        ...
        this.srot_measures = d3$1.set(this.initial_data.map(function (d) {
    ...
    function defineMeasureData() {
        ...
        if (this.config.y.type === 'log') {
            ...
            if (nonPositiveResults > 0) this.controls.wrap.selectAll('.axis-type .radio').filter(function () {
                return d3$1.select(this).select('input').attr('value') === 'log';
        ...
        this.nested_measure_data = d3$1.nest().key(function (d) {
            ...
            ['min', ['quantile', 0.05], ['quantile', 0.25], 'median', ['quantile', 0.75], ['quantile', 0.95], 'max', 'mean', 'deviation'].forEach(function (item) {
                ...
                results[stat] = Array.isArray(item) ? d3$1[fx](results.values, item[1]) : d3$1[fx](results.values);
            });
    ...
    function removeVisitsWithoutData() {
        ...
        if (!this.config.visits_without_data) this.config.x.domain = this.config.x.domain.filter(function (visit) {
            return d3$1.set(_this.filtered_measure_data.map(function (d) {
                return d[_this.config.time_settings.value_col];
    ...
    function setYdomain() {
        ...
        //Define y-domain.
        if (this.currentMeasure !== this.previousMeasure || this.currentYAxis !== this.previousYAxis) this.config.y.domain = d3$1.extent(this.measure_data.map(function (d) {
    ...
    function setLegendLabel() {
        this.config.legend.label = this.config.color_by !== 'srot_none' ? this.config.groups[this.config.groups.map(function (group) {
    ...
    function updateParticipantCount() {
        ...
        var subpopulationCount = d3$1.set(this.filtered_data.map(function (d) {
            return d[_this.config.id_col];
        })).values().length;
        var percentage = d3$1.format('0.1%')(subpopulationCount / this.populationCount);
        this.populationCountContainer.html('\n' + subpopulationCount + ' of ' + this.populationCount + ' participants  shown (' + percentage + ')<br>' + this.filtered_data.length + ' results');
    }
    ...
    function addYAxisTicks() {
        ...
        if (this.config.y.type === 'log') {
            ...
            this.svg.selectAll('.y.axis .tick').each(function (d) {
                ...
                if (tickValues.indexOf(tickValue) < 0) tickValues.push(tickValue);else tick.remove();
            });
        }
    }
    ...
    function addYAxisTicks$1() {
        ...
        if (!this.svg.selectAll('.y .tick')[0].length) {
            ...
            for (var i = 0; i < probs.length; i++) {
                probs[i].quantile = d3$1.quantile(this.measure_data.map(function (d) {
                    return +d[_this.config.y.column];
                }).sort(function (a, b) {
                    return a - b;
                }), probs[i].probability);
    ...
    function defineScales(subgroup) {
          ...
          subgroup.boxplot.y = this.config.y.type === 'linear' ? d3$1.scale.linear().range([this.plot_height, 0]).domain(this.y.domain()) : d3$1.scale.log().range([this.plot_height, 0]).domain(this.y.domain());
    }
    ...
    function defineData(subgroup) {
        ...
        subgroup.violinPlot = {
            histogram: d3$1.layout.histogram().bins(10).frequency(0)
        };
    ...
    function defineScales$1(subgroup) {
        ...
        subgroup.violinPlot.x = this.config.y.type === 'linear' ? d3$1.scale.linear().domain(this.y.domain()).range([this.plot_height, 0]) : d3$1.scale.log().domain(this.y.domain()).range([this.plot_height, 0]);
        subgroup.violinPlot.y = d3$1.scale.linear().domain([0, Math.max(1 - 1 / subgroup.group.x.nGroups, d3$1.max(subgroup.violinPlot.data, function (d) {
            return d.y;
    ...
    function addContainer$1(subgroup) {
        ...
        subgroup.violinPlot.area = d3$1.svg.area().interpolate('basis').x(function (d) {
            return subgroup.violinPlot.x(d.x + d.dx / 2);
        }).y0(subgroup.violinPlot.width / 2).y1(function (d) {
            return subgroup.violinPlot.y(d.y);
        });
        subgroup.violinPlot.line = d3$1.svg.line().interpolate('basis').x(function (d) {
            return subgroup.violinPlot.x(d.x + d.dx / 2);
        }).y(function (d) {
            return subgroup.violinPlot.y(d.y);
        });
