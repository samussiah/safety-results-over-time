export default function addBoxplot(chart, group
    ,boxInsideColor = '#eee'
    ,precision = 0) {

  //Make the numericResults numeric and sort.
    const numericResults = group.results
        .map(d => + d)
        .sort(d3.ascending);
    const boxPlotWidth = .75/chart.colorScale.domain().length;
  ...
  //Define box plot container.
    const boxplot = group.svg.append('g')
        .attr('class','boxplot')
        .datum(
