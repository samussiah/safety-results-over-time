export default function onDataTransform() {
    ...
    const group_labels = this.config.groups.map(
        group => (group.label ? group.label : group.value_col ? group.value_col : group)
    );
    const group = this.config.color_by;

    if (group !== 'NONE') this.config.legend.label = group_labels[group_value_cols.indexOf(group)];
